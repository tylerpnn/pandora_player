...
public class MenuBar extends JMenuBar implements ActionListener {
	...
	public MenuBar(final Frame frame) {
		...
		muteAds = new JCheckBoxMenuItem("Mute Ads", Frame.getConfig().getMuteAds());
		muteAds.addActionListener(this);		
		compact = new JCheckBoxMenuItem("Compact mode", Frame.getConfig().isCompact());
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == login) {
			if(!isLoggedIn) {
				...
				if(config.getRememberUser() && config.getUser() != null) {
					frame.login(config.getUser(), 
							config.getProxy());
				} else {
		...
		if(e.getSource() == muteAds) {
			Frame.getConfig().setMuteAds(muteAds.getState());
		}
